...
public class StepView extends View {
    ...
    public StepView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textSize = (int) ta.getDimension(R.styleable.FlowViewVertical_v_textsize, 14);
        ta.recycle();
        init();
    }
    ...
    private void drawText(Canvas canvas) {
        for (int i = 0; i < maxStep; i++) {
            ...
            if (null != titles) {
                ...
                StaticLayout sl = new StaticLayout(Html.fromHtml(titles[i]), textPaint, border, Layout.Alignment.ALIGN_NORMAL, 1.1f, 0.0f, false);
                sl.draw(canvas);
                canvas.restore();
            }
        }
